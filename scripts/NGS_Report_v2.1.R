...
utils::globalVariables(c(
  ...
  "Circular_contigs",
  "Contig_Num", "N50", "GC_percent", "Largest_Contig", "X2", "Support", 
...
read_mlst_st <- function(tsv_path) {
  ...
  df <- tryCatch(utils::read.delim(tsv_path, sep = "\t", header = FALSE, stringsAsFactors = FALSE, quote = "", comment.char = "", fill = TRUE, check.names = FALSE), error = function(e) NULL)
...
read_rmlst_taxon <- function(tsv_path) {
  if (!file.exists(tsv_path)) return(NA_character_)
  df <- tryCatch(readr::read_tsv(tsv_path, show_col_types = FALSE, progress = FALSE), error = function(e) NULL)
...
read_amr_column <- function(txt_path, scope_wanted, type_wanted, subtype_wanted = NULL, out_col = "Element symbol") {
  ...
  df <- tryCatch(readr::read_tsv(txt_path, show_col_types = FALSE, progress = FALSE), error = function(e) NULL)
  if (is.null(df) || nrow(df) == 0) return(NA_character_)
...
get_pipeline_versions <- function(logs_dir) {
  ...
  for (t in names(versions_list)) {
      ...
      if (!is.na(entry$db)) {
          if (str != "") str <- paste0(str, " (DB: ", entry$db, ")")
          else str <- paste0("DB: ", entry$db)
      }
...
final_columns <- c(
  ...
  "ARGs (>90% cov, >90% ID, AMRFinderPlus)", "Point Mutations", "Predicted Phenotype", "Virulence Genes (>90% cov, >90% ID, AMRFinderPlus)", 
  "Comments"
)
...
circular_contigs_col <- tibble(`Circular Contigs` = purrr::map_chr(base_part$Lab_ID, function(iso_id) {
    ...
    candidate <- find_isolate_file(ASSEMBLY_DIR, iso_id, suffix_regex = "ASSEMBLY_INFO", platform = platform_search)
...
calculate_stats <- function(report, logs_dir) {
  ...
  list(
    ...
    depth_high = sum(sapply(report$Depth, function(x) { num <- suppressWarnings(as.numeric(gsub("x", "", x))); !is.na(num) && num > 30 }), na.rm = TRUE),
    circular_contigs = sum(sapply(report$`Circular Contigs`, function(x) { num <- suppressWarnings(as.numeric(x)); !is.na(num) && num > 0 }), na.rm = TRUE),
...
r_script_dir <- dirname(sub("--file=", "", commandArgs(trailingOnly = FALSE)[grep("--file=", commandArgs(trailingOnly = FALSE))]))
if (length(r_script_dir) == 0) r_script_dir <- getwd()
js_file <- file.path(r_script_dir, "generate_html.js")

if (file.exists(js_file)) {
  js_command <- sprintf('node "%s" "%s" "%s" "%s"', js_file, BASE_DIR, json_data_file, json_stats_file)
