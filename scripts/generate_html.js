// generate_html.js (FULL FILE)
// Changes included:
// 1) Clearer card/table titles for PlasmidFinder + MOB-suite (3 cols) + AMRFinderPlus + rMLST
// 2) Hide MOB-suite 3 columns in main table (like plasmids/args) -> only visible in expanded details cards
// 3) Revised footer section (versions + "Report guide") with wording you asked for
// Everything else kept as close as possible to your original file.
...
try {
  ...
  const outputFile = path.join(BASE_DIR, 'final_report.html');
...
function createAdvancedHTML(reportData, stats, versionsText) {
  ...
  const headerMap = {
    "Plasmid (PlasmidFinder)": "Plasmids (PlasmidFinder)",
    "Mobility (MOBTyper)": "Mobility (MOB-suite)",
    "MPF (MOBTyper)": "Conjugation system (MPF, MOB-suite)",
    "Cluster (MOBTyper)": "Plasmid cluster (MOB-suite)",

    "ARGs (>90% cov, >90% ID, AMRFinderPlus)": "ARGs (AMRFinderPlus)",
    "Virulence Genes (>90% cov, >90% ID, AMRFinderPlus)": "Virulence genes (AMRFinderPlus)",
    "Point Mutations": "Point mutations (AMRFinderPlus)",
    "Predicted Phenotype": "Predicted phenotype (AMRFinderPlus)",

    "Detected Organism": "Detected organism (rMLST)",
    "Detection (rMLST)": "rMLST support",
  ...
  const indices = {
    ...
    genomeLen: dataKeys.findIndex(k => k.toLowerCase().includes("genome length") || k.toLowerCase().includes("genome size") || k.toLowerCase().includes("genome")),
    ...
    plasmids: dataKeys.findIndex(k => k.toLowerCase().includes("plasmid (plasmidfinder)") || (k.toLowerCase().includes("plasmid") && k.toLowerCase().includes("finder"))),

    // MOB-suite 3 cols
    mobility: dataKeys.findIndex(k => k.toLowerCase().includes("mobility (mobtyper)")),
    mpf: dataKeys.findIndex(k => k.toLowerCase().includes("mpf (mobtyper)")),
    cluster: dataKeys.findIndex(k => k.toLowerCase().includes("cluster (mobtyper)")),
...
function generateHtmlTemplate(tableHeaders, tableRows, stats, idx) {
  ...
  if (stats.versions && Object.keys(stats.versions).length > 0) {
    versionsHtml += '<h4><i class="fas fa-code-branch"></i> Versions used in this run</h4><ul class="version-list">';
...
<head>
  ...
  <style>
    ...
    .stat-value { font-size: 2.0em; font-weight: 700; margin: 10px 0; }
...
<div class="container">
  ...
  <div class="footer-legend">
    ...
    <h4><i class="fas fa-info-circle"></i> Report guide</h4>
    <ul>
      <li><strong>QC flags</strong>: Depth &lt;30×, rMLST support &lt;95%, or Expected ≠ Detected organism (excluding “N/A (Unknown)”).</li>
      <li><strong>Plasmids (PlasmidFinder)</strong>: replicon-based hits (may miss some <em>Pseudomonas</em> backbones).</li>
      <li><strong>Mobility (MOB-suite)</strong>: Mobility status, MPF type, and primary cluster ID (plasmid lineage).</li>
      <li><strong>AMR/Virulence (AMRFinderPlus)</strong>: filtered at ≥90% coverage and ≥90% identity.</li>
      <li><strong>rMLST</strong>: depends on PubMLST REST API availability; timeouts may yield empty results.</li>
    </ul>
  </div>
</div>
...
$(document).ready(function() {
  ...
  // Hide MOB-suite columns in main table (show only in details)
  if (IDX.mobility !== -1) columns_hidden_details.push(IDX.mobility);
  if (IDX.mpf !== -1) columns_hidden_details.push(IDX.mpf);
  if (IDX.cluster !== -1) columns_hidden_details.push(IDX.cluster);
    ...
    columnDefs: [
...

      {
        ...
        createdCell: function(td, cellData, rowData) {
          ...
          if (expected === "n/a (unknown)") return;
    ...
    if (IDX.q30 !== -1) {
      ...
      filteredData.forEach(row => {
        ...
        if (val) {
          ...
          if (!isNaN(val)) { totalQ30 += val; countQ30++; }
        }
      });
      if (countQ30 > 0) $("#mean-q30").text((totalQ30 / countQ30).toFixed(2) + "%");
      else $("#mean-q30").text("N/A");
    }
  }
