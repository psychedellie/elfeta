...
workflow LP_PIPELINE {
    ...
    main:
        ...
        def hybrid_input = hq_reads_ont
            .join(hq_reads_iln)
            .map { sample_id, long_reads, read1, read2 ->
                tuple(sample_id, long_reads, read1, read2)
            }

        hybrid_input.view { sample_id, long_reads, read1, read2 ->
            "Hybrid Input - Sample: ${sample_id} | ONT: ${long_reads.name} | R1: ${read1.name}, R2: ${read2.name}"
        }
        ...
        def polypolish_in = unicycler_out.fasta.join(filter_out.sams).map { sample_id, assembly_fasta, filtered_sam1, filtered_sam2 ->
