...
workflow LP_PIPELINE {
    ...
    def hybrid_input = hq_reads_ont.join(hq_reads_iln)
    
    // Initial Assembly
    def unicycler_out = UNICYCLER(hybrid_input, params.unicycler)

    // Medaka Polishing
    def medaka_in = hq_reads_ont.join(unicycler_out.fasta)
    ...
    def final_assembly = POLYPOLISH_POLISH(
        unicycler_out.fasta.join(filter_out.sams), 
    ...
    // Typing & Annotation
    def quast_out = QUAST_ILN(hq_reads_iln.join(final_assembly), params.quast)
    ...
    // Use Unicycler log for assembly metrics
    def assembly_info_file = unicycler_out.log
